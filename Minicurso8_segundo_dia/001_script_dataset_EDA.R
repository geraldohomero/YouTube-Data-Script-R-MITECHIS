...
data_news_gaza_israel <- readtext("news_gaza_israel/*.txt") |> 
  ...
  mutate(videoId = substr(videoId, 1, 11),
         ...
         text = str_replace_all(text, c(" gaz " = " gaza "))) |> # Substitui palavras alteradas pela Lematização para o termo correto
...
treemap_paises_news_gaza_israel <- ggplot(paises_news_gaza_israel, aes(
  ...
  labs(  # Adiciona rótulos ao gráfico
    title = "Composição da amostra das notícias\npor país dos jornais",  # Título do gráfico
    subtitle = "Amostra agregada dos anos de 2009 a 2024",  # Subtítulo do gráfico
    caption = "Fonte: Tuguna Digital Lab - Laboratório de Humanidades Digitais\nUEMG Barbacena"  # Fonte dos dados
